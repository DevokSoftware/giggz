...
public class YoutubeDiscover {
    ...
    public void syncLatestVideos() {
        try {
            ...
            for (FeedChannel channel : channels) {
                if (channel.getLastSync().plusMinutes(channel.getSyncPeriod()).isAfter(OffsetDateTime.now())) {
                    continue;
                }
    ...
    private List<FeedContentDTO> getRecentVideos(YouTube youtube, String playlistId, FeedChannel channel) throws IOException {
        ...
        excludeExistingVideos(playlistResponse);
        return mapFeedContentResponse(channel, playlistId, playlistResponse);
    }

    private List<FeedContentDTO> mapFeedContentResponse(FeedChannel channel, String playlistId, PlaylistItemListResponse playlistResponse) {
        return playlistResponse.getItems().stream()
                .filter(item -> {
                    String url = "https://www.youtube.com/watch?v=" + item.getSnippet().getResourceId().getVideoId();
                    return !feedContentService.existsByUrl(url);
                })
                .map(item -> {
                    FeedContentDTO dto = new FeedContentDTO();
                    dto.setSource(SourceType.YOUTUBE);
                    dto.setFeedChannel(channel);
...
//                    dto.setDuration(item.getSnippet());
                    ...
                    if (publishedAt != null) {
                        ...
                        dto.setPublishDate(offsetDateTime);
                    }
    ...
    private void excludeExistingVideos(PlaylistItemListResponse playlistResponse) {
        for (PlaylistItem playlistItem : playlistResponse.getItems()) {
            String url = "https://www.youtube.com/watch?v=" + playlistItem.getSnippet().getResourceId().getVideoId();
